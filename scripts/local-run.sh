...
JWT_TTL_SECONDS="$(resolve_setting CVECT_JWT_TTL_SECONDS 86400)"
...
EMBEDDING_TIMEOUT_SECONDS="$(resolve_setting CVECT_EMBEDDING_TIMEOUT_SECONDS 120)"
...
STORAGE_TYPE="$(resolve_setting CVECT_STORAGE_TYPE local)"
STORAGE_LOCAL_ROOT="$(resolve_setting CVECT_STORAGE_LOCAL_ROOT /tmp/cvect-storage)"
SHOW_SQL="$(resolve_setting CVECT_SHOW_SQL false)"
VECTOR_MAX_CONCURRENT_WRITES="$(resolve_setting CVECT_VECTOR_MAX_CONCURRENT_WRITES 2)"
...
start_backend() {
  ...
  (
    ...
    nohup env \
      ...
      CVECT_JWT_TTL_SECONDS="${JWT_TTL_SECONDS}" \
      CVECT_CACHE_ENABLED="${CACHE_ENABLED}" \
      CVECT_SHOW_SQL="${SHOW_SQL}" \
      CVECT_VECTOR_ENABLED="${VECTOR_ENABLED}" \
      CVECT_VECTOR_MAX_CONCURRENT_WRITES="${VECTOR_MAX_CONCURRENT_WRITES}" \
      CVECT_VECTOR_INGEST_WORKER_ENABLED="${VECTOR_INGEST_WORKER_ENABLED}" \
      CVECT_UPLOAD_WORKER_ENABLED="${UPLOAD_WORKER_ENABLED}" \
      CVECT_STORAGE_TYPE="${STORAGE_TYPE}" \
      CVECT_STORAGE_LOCAL_ROOT="${STORAGE_LOCAL_ROOT}" \
      ...
      CVECT_EMBEDDING_TIMEOUT_SECONDS="${EMBEDDING_TIMEOUT_SECONDS}" \
